...
namespace ECRControllerSim;

static class Program
{
    private static readonly string _logFile = Path.Combine(AppContext.BaseDirectory, "ECRSim.log");

    private static EcrController _controller;
    ...
    static void Main()
    {
        ...
        _controller = new EcrController(
            new EcrClient(http, new Uri($"{url}:{port}")),
    ...
    private static async Task SendSelectedJson()
    {
        ...
        {
            var dto = j.ToObject<WebhookRegistration>()!;
            result = await _controller.RegisterWebhook(dto);
        }
        else
        {
            var dto = j.ToObject<WebhookUnregistration>()!;
