...
public class Client : IClient
{
    ...
    public Client(HttpClient http, Uri baseUri)
    {
    ...
    public Task<RequestResult> InitiateSession(InitiateSessionRequest request, CancellationToken ct = default)
    {
        if (request == null)
            return Fail("request is null");

        return PostAsync(ApiPaths.SessionInitiate, request);
    }

    public Task<RequestResult> CompleteSession(CompleteSessionRequest request, CancellationToken ct = default)
    {
        if (request == null)
            return Fail("request is null");

        return PostAsync(ApiPaths.SessionComplete, request);
    }

    public Task<RequestResult> StartTransaction(StartTransactionRequest request, TestRunInformation testRunInformation, CancellationToken ct)
    {
        if (request == null)
            return Fail("request is null");
    ...
    public Task<RequestResult> AbortTransaction(AbortTransactionRequest req, CancellationToken ct)
    {
        if (req == null)
    ...
    public Task<RequestResult<StatusResponse>> GetStatus(string terminalId, CancellationToken ct)
    {
    ...
    public Task<RequestResult> RegisterWebhook(RegisterWebhookRequest req, CancellationToken ct)
    {
        if (req == null)
            return Fail("req is null");
    ...
    public Task<RequestResult> UnregisterWebhook(UnregisterWebhookRequest req, CancellationToken ct)
    {
        if (req == null)
            return Fail("req is null");
