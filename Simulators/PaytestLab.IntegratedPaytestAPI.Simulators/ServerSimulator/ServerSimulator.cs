...
namespace PaytestLab.IntegratedPaytestAPI;
...
internal class ServerSimulator
{
    ...
    static readonly ConcurrentDictionary<string, string> _activeSessions = new();
    ...
    static readonly string _logFilePath = Path.Combine(AppContext.BaseDirectory, "ServerLogs.txt");
    ...
    static async Task HandleRequest(HttpListenerContext ctx)
    {
        try
        {
            ...
            switch (path)
            {
                case "/session/initiate" when method == "POST": await OnInitiateSession(ctx); break;
                case "/session/complete" when method == "POST": await OnCompleteSession(ctx); break;
    ...
    static async Task OnInitiateSession(HttpListenerContext ctx)
    {
        var body = await ReadAndLogRequest(ctx);
        InitiateSessionRequest req;
        try
        {
            req = JsonConvert.DeserializeObject<InitiateSessionRequest>(body, _jsonSettings)!;
        }
        catch (Exception ex)
        {
            Log($"[ERR] InitiateSessionRequest JSON: {ex.Message}");
        ...
        {
            ctx.Response.StatusCode = 400;
            Write(ctx.Response, new { message = "terminalid is missing" });
            return;
        }

        var sessionId = Guid.NewGuid();

        if (!_activeSessions.TryAdd(req.TerminalId, sessionId.ToString()))
        {
            ctx.Response.StatusCode = 409;
            Write(ctx.Response, new { message = $"Session already active for terminalid {req.TerminalId}" });
            return;
        }

        ctx.Response.StatusCode = 200;
        Write(ctx.Response, new InitiateSessionResponse() { SessionId = sessionId.ToString() });
        Log($"[SIM] Session {sessionId} started");
    }
    ...
    static async Task OnStart(HttpListenerContext ctx)
    {
        ...
        catch (Exception ex)
        {
            ...
            ctx.Response.StatusCode = 400;
            Write(ctx.Response, new { message = ex.Message });
            return;
        }

        if (!_activeSessions.TryGetValue(req.TerminalId, out var activeSessionId))
        {
            ctx.Response.StatusCode = 400;
            Write(ctx.Response, new { message = $"No active session for terminalid {req.TerminalId}" });
            return;
        }
        ...
        {
            ctx.Response.StatusCode = 400;
            Write(ctx.Response, new { message = $"SessionId {req.SessionId} is not active for terminalid {req.TerminalId}" });
            return;
        }
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                var notif = new TransactionCompletionNotification
                {
                    ...
                    AuthorizedAmount = new Amount("EUR", 10.00m),
                    TenderName = "Visa",
                    PosEntry = "Chip",
    ...
    static async Task OnCompleteSession(HttpListenerContext ctx)
    {
        var body = await ReadAndLogRequest(ctx);
        CompleteSessionRequest req;
        try
        {
            req = JsonConvert.DeserializeObject<CompleteSessionRequest>(body, _jsonSettings);
        }
        catch (Exception ex)
        {
            Log($"[ERR] InitiateSessionRequest JSON: {ex.Message}");
        ...
        var session = _activeSessions
            .FirstOrDefault(x => x.Value == req.SessionId);

        if (session.Key is null)
        {
            ctx.Response.StatusCode = 404;
            Write(ctx.Response, new { message = "SessionId not found or already completed" });
            return;
        }
    ...
    static async Task OnRegister(HttpListenerContext ctx)
    {
        ...
        try
        {
            req = JsonConvert.DeserializeObject<RegisterWebhookRequest>(body, _jsonSettings);
        }
    ...
    {
        var body = await new StreamReader(ctx.Request.InputStream, Encoding.UTF8).ReadToEndAsync();
        UnregisterWebhookRequest req;
        try
        {
            req = JsonConvert.DeserializeObject<UnregisterWebhookRequest>(body, _jsonSettings);
        }
        catch (JsonException ex)
        {
